...
void BMI270Sensor::internal_setup_(int stage, int retry) {
  ...
  switch (stage) {
    ...
    case 3: {
      ...
      if (!write_register_(BMI2_PWR_CTRL_ADDR, &temp_en)){
...
uint8_t BMI270Sensor::getBMI270_AccRange() {
  ...
  // 1. Pointer vers le registre 0x41
  Wire.beginTransmission(BMI2_I2C_PRIM_ADDR); // Adresse I2C du BMI270
  Wire.write(BMI2_ACC_RANGE_ADDR);             // Adresse du registre ACC_RANGE
  if (Wire.endTransmission(false) != 0) { // 'false' pour un "Restart"
    return 0xFF; 
  }
